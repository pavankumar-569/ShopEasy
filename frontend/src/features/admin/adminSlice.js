...
export const fetchProductReviews=createAsyncThunk('admin/fetchProductReviews',async(productId,{rejectWithValue})=>{
  try{
   ...
   const {data}=await axios.get(`/api/v1/adminPdashb/reviews?id=${productId}`);
   return data;
  }catch(error){
...
export const deleteReview=createAsyncThunk('admin/deleteReview',async({productId,reviewId},{rejectWithValue})=>{
  try{
   ...
   const {data}=await axios.delete(`/api/v1/adminPdashb/reviews?productId=${productId} & id=${reviewId}`);
   return data;
  }catch(error){
...
const adminSlice=createSlice({
    ...
    extraReducers:(builder)=>{
        ...
        .addCase(updateUserRole.rejected,(state,action)=>{
            state.loading=false;
            state.error=action.payload?.message || "Failed to fetch update user role ";
        })
